...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resize(){
        ...
        const actionFontSize = buttonWidth * 0.45;
        const menuFontSize = buttonWidth * 0.60;
    ...
    function showActionButtons(menuType){menuType==="towers"?(towerActionButtons.forEach(t=>t.classList.remove("hidden")),monsterActionButtons.forEach(t=>t.classList.add("hidden")),towerMenuBtn.classList.add("active"),monsterMenuBtn.classList.remove("active")):(towerActionButtons.forEach(t=>t.classList.add("hidden")),monsterActionButtons.forEach(t=>t.classList.remove("hidden")),monsterMenuBtn.classList.add("active"),towerMenuBtn.classList.remove("active")),allActionButtons.forEach(t=>t.classList.remove("selected")),selectedAction=null}
    function handleActionButtonClick(btn){const o=btn.dataset.type,n=btn.dataset.unit;allActionButtons.forEach(t=>t.classList.remove("selected")),btn.classList.add("selected"),selectedAction={type:o,unit:n},"tower"===o&&log(`Modo construção: ${n}.`),"monster"===o&&(startGameIfNeeded(),playerActions.push({action:"spawn",type:n,timestamp:roundTime}),log(`Ação Gravada: Enviar ${n}`))}
    function handleCanvasClick(e){if(!selectedAction||"tower"!==selectedAction.type)return;const o=canvas.getBoundingClientRect(),n=e.clientX-o.left,c=e.clientY-o.top,{col:a,row:r}=screenToGrid(n,c),i=a>=0&&r>=15&&!path.some(t=>t.x===a&&t.y===r)&&!towers.some(t=>t.col===a&&t.row===r)&&gold>=100;i?(startGameIfNeeded(),updateGold(-100),towers.push(new Tower(a,r)),playerActions.push({action:"build",type:selectedAction.unit,col:a,row:r,timestamp:roundTime}),log(`Torre ${selectedAction.unit} construída.`)):log("Construção inválida."),allActionButtons.forEach(t=>t.classList.remove("selected")),selectedAction=null}
    ...
    async function saveGhost(actions) {
        ...
        try {
            ...
            log(`Ghost guardado com sucesso no Firestore com o ID: ${docRef.id}`);
            alert("Ronda vitoriosa! O seu 'Ghost' foi guardado para desafiar outros jogadores.");
        } catch (error) {
            console.error("Erro ao guardar o Ghost:", error);
            alert("Houve um erro ao guardar o seu Ghost. Por favor, tente novamente.");
        }
    }
    ...
    function endGame(isVictory) {
        ...
        if (isVictory) {
            log("Vitória! A guardar Ghost...");
            saveGhost(playerActions);
        } else {
            log("Derrota!");
            alert("Fim de jogo! Foi derrotado.");
        }

        // Delay para o jogador ver a mensagem antes de reiniciar
        setTimeout(() => {
            resetGame();
