...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    let playerGold, playerHealth, monsters, towers, playerActions, selectedAction, hoveredTower;
    ...
    // ============== CICLO DE JOGO ==============
    function startGameIfNeeded(){gameStarted||(gameStarted=!0,requestAnimationFrame(gameLoop))}
    function gameLoop(timestamp){lastTime||(lastTime=timestamp);const dT=Math.min((timestamp-lastTime)/1e3,1/30);lastTime=timestamp,gameStarted&&(roundTime+=dT,updatePlayerGold(1*dT),ghost&&updateGhostGold(1*dT),ghost&&-1!==nextGhostActionIndex&&(()=>{for(;nextGhostActionIndex<ghostActions.length&&roundTime>=ghostActions[nextGhostActionIndex].timestamp;){const t=ghostActions[nextGhostActionIndex];"spawn"===t.action?spawnGhostMonster(t.type,t.level):"build"===t.action&&buildGhostTower(t.type,t.level,t.col,t.row),nextGhostActionIndex++}})(),monsters.forEach(t=>t.move(dT)),ghostMonsters.forEach(t=>t.move(dT)),towers.forEach(t=>{t.findTarget(ghostMonsters),t.attack(dT)}),ghostTowers.forEach(t=>{t.findTarget(monsters),t.attack(dT)}),(()=>{const t=monsters.filter(t=>t.reachedEnd);t.length>0&&updateGhostHealth(-10*t.length)})(),(()=>{const t=ghostMonsters.filter(t=>t.reachedEnd);t.length>0&&updatePlayerHealth(-10*t.length)})(),monsters=monsters.filter(t=>t.health>0&&!t.reachedEnd),ghostMonsters=ghostMonsters.filter(t=>t.health>0&&!t.reachedEnd),ghost&&nextGhostActionIndex>=ghostActions.length&&0===monsters.length&&0===ghostMonsters.length&&endGame(!0)),drawGrid(),towers.forEach(t=>t.draw()),ghostTowers.forEach(t=>t.draw()),monsters.forEach(t=>t.draw()),ghostMonsters.forEach(t=>t.draw()),hoveredTower&&(ctx.beginPath(),ctx.arc(hoveredTower.x,hoveredTower.y,hoveredTower.range,0,2*Math.PI),ctx.fillStyle="rgba(255, 255, 255, 0.2)",ctx.fill()),selectedAction&&"tower"===selectedAction.type&&selectedAction.ghostTower.visible&&(()=>{const{x,y,range,isValid}=selectedAction.ghostTower;ctx.beginPath(),ctx.arc(x,y,10,0,2*Math.PI),ctx.fillStyle=isValid?"rgba(0, 100, 255, 0.5)":"rgba(255, 0, 0, 0.5)",ctx.fill(),ctx.beginPath(),ctx.arc(x,y,range,0,2*Math.PI),ctx.fillStyle=isValid?"rgba(0, 100, 255, 0.2)":"rgba(255, 0, 0, 0.2)",ctx.fill()})(),requestAnimationFrame(gameLoop)}
    ...
    async function resetGame(){gameStarted=!1,playerGold=500,playerHealth=100,monsters=[],towers=[],playerActions=[],ghostGold=500,ghostHealth=100,ghostTowers=[],ghostMonsters=[],roundTime=0,selectedAction=null,hoveredTower=null,lastTime=null,updatePlayerGold(0),updatePlayerHealth(0),updateGhostGold(0),updateGhostHealth(0),await loadGhost(),nextGhostActionIndex=ghost?0:-1,showActionButtons('towers'),resize(),requestAnimationFrame(gameLoop)}
