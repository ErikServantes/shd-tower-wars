// ============== Echoes of Evolution - Core Logic ==============
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Inicializa apenas se não houver instâncias ativas
    ...
    const unitTranslation = {
        "goblin": "swordsman", "orc": "knight", "golem": "battering_ram", "bat": "hydrogen_balloon",
        "skeleton": "specialist", "wolf": "knight", "dragon": "nokfit_berserker",
        "arrow": "giant_crossbow", "mage": "oil_launcher", "cannon": "catapult",
        "slow": "oil_launcher", "sniper": "steampunk_sniper", "splash": "sonic_cannon", "farm": "farm"
    };
    ...
    function updatePlayerGold(amount) {
        playerGold += amount;
        if (playerGoldSpan) playerGoldSpan.textContent = Math.floor(playerGold);
    }
    function updateGhostGold(amount) {
        ghostGold += amount;
        if (enemyGoldSpan) enemyGoldSpan.textContent = Math.floor(ghostGold);
    }
    function updatePlayerHealth(amount) {
        playerHealth += amount;
        if (playerHpSpan) playerHpSpan.textContent = Math.max(0, Math.floor(playerHealth));
        if (playerHealth <= 0) endGame(false);
    }
    function updateGhostHealth(amount) {
        ghostHealth += amount;
        if (enemyHpSpan) enemyHpSpan.textContent = Math.max(0, Math.floor(ghostHealth));
        if (ghostHealth <= 0) endGame(true);
    }
    ...
    async function loadGameData() {
        try {
            const v = `?v=${Date.now()}`;
            const [tRes, mRes] = await Promise.all([ fetch(`towers.json${v}`), fetch(`monsters.json${v}`) ]);
            towerData = await tRes.json(); monsterData = await mRes.json();
    ...
    class Camera {
        ...
        project(c, r) { const tW = this.canvas.width / 15, tH = this.canvas.height / 30; return { x: c * tW, y: r * tH, tileWidth: tW, tileHeight: tH }; }
        screenToGrid(sX, sY) { const tW = this.canvas.width / 15, tH = this.canvas.height / 30; const col = Math.floor(sX / tW), row = Math.floor(sY / tH); return (col < 0 || col >= 15 || row < 0 || row >= 30) ? { col: -1, row: -1 } : { col, row }; }
        getTileCenter(c, r) { const p = this.project(c, r); return { x: p.x + p.tileWidth / 2, y: p.y + p.tileHeight / 2 }; }
    }

    class FloatingText {
        constructor(x, y, text, color) { this.x = x; this.y = y; this.text = text; this.color = color; this.life = 1.0; this.velocity = -20; }
        update(dt) { this.life -= dt; this.y += this.velocity * dt; }
        draw() { let opacity = this.life > 0.8 ? (1 - this.life) / 0.2 : this.life < 0.3 ? this.life / 0.3 : 1; ctx.globalAlpha = Math.max(0, opacity); ctx.fillStyle = this.color; ctx.font = "bold 20px Arial"; ctx.textAlign = "center"; ctx.fillText(this.text, this.x, this.y); ctx.globalAlpha = 1.0; ctx.textAlign = "start"; }
    }

    class Projectile {
        constructor(x, y, target, damage, owner, damageType) { this.x = x; this.y = y; this.target = target; this.damage = damage; this.owner = owner; this.speed = 400; this.damageType = damageType || 'normal'; }
        move(dT) { if (!this.target || this.target.health <= 0) return; const dX = this.target.x - this.x, dY = this.target.y - this.y, dist = Math.sqrt(dX * dX + dY * dY), moveDist = this.speed * dT; if (dist < moveDist) { this.x = this.target.x; this.y = this.target.y; } else { this.x += (dX / dist) * moveDist; this.y += (dY / dist) * moveDist; } }
        draw() { ctx.fillStyle = "yellow"; ctx.beginPath(); ctx.arc(this.x, this.y, 3, 0, 2 * Math.PI); ctx.fill(); }
    }

    class Monster {
        constructor(t, l, p, owner) {
            const tr = translateUnit(t), c = monsterData[tr].levels[l - 1];
            this.type = tr; this.level = l; this.isFlying = c.isFlying || false; this.stealth = c.stealth || false;
            this.immuneToNormal = c.immuneToNormal || false; this.damageToPlayer = c.damageToPlayer || 1;
            this.path = this.isFlying ? (p === playerPath ? playerFlyingPath : ghostFlyingPath) : p;
            this.pathIndex = 0; this.health = c.health; this.maxHealth = c.health; this.speed = c.speed; this.reward = c.reward; this.owner = owner;
            const s = camera.getTileCenter(this.path[0].x, this.path[0].y); this.x = s.x; this.y = s.y; this.reachedEnd = false;
        }
        move(dT) { if (this.reachedEnd) return; let moveDist = this.speed * dT; while (moveDist > 0 && !this.reachedEnd) { if (this.pathIndex >= this.path.length) { this.reachedEnd = true; break; } const tc = camera.getTileCenter(this.path[this.pathIndex].x, this.path[this.pathIndex].y), dX = tc.x - this.x, dY = tc.y - this.y, d = Math.sqrt(dX * dX + dY * dY); if (d < 0.1) { this.pathIndex++; continue; } if (moveDist >= d) { this.x = tc.x; this.y = tc.y; this.pathIndex++; moveDist -= d; } else { this.x += (dX / d) * moveDist; this.y += (dY / d) * moveDist; moveDist = 0; } } }
        draw() { ctx.fillStyle = "rgba(0,0,0,0.3)"; ctx.beginPath(); ctx.ellipse(this.x, this.y + 8, 8, 4, 0, 0, 2 * Math.PI); ctx.fill(); if (this.stealth) ctx.globalAlpha = 0.5; ctx.fillStyle = this.owner === 'player' ? '#2ecc71' : '#e74c3c'; ctx.beginPath(); ctx.arc(this.x, this.y, 10, 0, 2 * Math.PI); ctx.fill(); if (this.isFlying) { ctx.strokeStyle = 'cyan'; ctx.lineWidth = 2; ctx.stroke(); } if (this.health < this.maxHealth) { const p = this.y - 20, w = 30, h = 5; ctx.fillStyle = "#444"; ctx.fillRect(this.x - w / 2, p, w, h); ctx.fillStyle = "#0f0"; ctx.fillRect(this.x - w / 2, p, w * (this.health / this.maxHealth), h); } ctx.globalAlpha = 1.0; }
    }

    class Tower {
        constructor(t, l, c, r, owner) {
            const tr = translateUnit(t), d = towerData[tr].levels[l - 1];
            this.type = tr; this.level = l; this.col = c; this.row = r; this.damage = d.damage;
            this.damageType = towerData[tr].damageType || 'normal'; this.canSeeStealth = towerData[tr].canSeeStealth || false;
            this.canAttackFlying = d.canAttackFlying || false; this.canAttackGround = d.canAttackGround || false;
            const tw = camera.project(c, r).tileWidth; this.range = d.range * tw; this.fireRate = d.fireRate; this.cost = d.cost; this.owner = owner;
            const p = camera.getTileCenter(c, r); this.x = p.x; this.y = p.y; this.target = null; this.fireCooldown = 0;
        }
        findTarget(monsters) { if (this.target && this.target.health > 0 && Math.sqrt(Math.pow(this.x - this.target.x, 2) + Math.pow(this.y - this.target.y, 2)) <= this.range) { if (this.target.stealth && !this.canSeeStealth) { } else return; } this.target = null; let minD = Infinity; for (const m of monsters) { if ((m.isFlying && !this.canAttackFlying) || (!m.isFlying && !this.canAttackGround)) continue; if (m.stealth && !this.canSeeStealth) continue; const d = Math.sqrt(Math.pow(this.x - m.x, 2) + Math.pow(this.y - m.y, 2)); if (d <= this.range && d < minD) { minD = d; this.target = m; } } }
        ...
        draw() { ctx.fillStyle = "rgba(0,0,0,0.3)"; ctx.beginPath(); ctx.ellipse(this.x, this.y + 10, 12, 6, 0, 0, 2 * Math.PI); ctx.fill(); ctx.fillStyle = this.owner === 'player' ? '#3498db' : '#9b59b6'; ctx.beginPath(); ctx.arc(this.x, this.y, 12, 0, 2 * Math.PI); ctx.fill(); ctx.strokeStyle = "white"; ctx.lineWidth = 1; ctx.stroke(); }
    }
    ...
    function gameLoop(timestamp) {
        ...
        if (gameStarted && !isRoundOver) {
            updatePlayerGold(1 * dT); if (ghost) updateGhostGold(1 * dT);
            roundTimer -= dT; if (timerDisplaySpan) timerDisplaySpan.textContent = formatTime(roundTimer);
            if (roundTimer <= 0) { log("Tempo esgotado."); endRound(false); return; }
            if (monsters.length === 0 && ghostMonsters.length === 0) { timeWithoutMonsters += dT; } else { timeWithoutMonsters = 0; }
            if ((ROUND_DURATION - roundTimer) >= 30 && timeWithoutMonsters >= 5) { log("Empate técnico (Sem monstros)."); endRound(false); return; }

            const et = ROUND_DURATION - roundTimer;
            if (ghost && nextGhostActionIndex < roundGhostActions.length) {
                while (nextGhostActionIndex < roundGhostActions.length && et >= roundGhostActions[nextGhostActionIndex].timestamp) {
                    const a = roundGhostActions[nextGhostActionIndex];
                    if (a.action === 'spawn') spawnGhostMonster(a.type, a.level);
                    else if (a.action === 'build') buildGhostTower(a.type, a.level, 14 - a.col, 29 - a.row);
                    nextGhostActionIndex++;
                }
            }
            monsters.forEach(m => m.move(dT)); ghostMonsters.forEach(m => m.move(dT));
            towers.forEach(t => { t.findTarget(ghostMonsters); t.attack(dT); }); ghostTowers.forEach(t => { t.findTarget(monsters); t.attack(dT); });
            for (let i = projectiles.length - 1; i >= 0; i--) {
                const p = projectiles[i]; if (!p.target || p.target.health <= 0) { projectiles.splice(i, 1); continue; }
                p.move(dT); const dX = p.target.x - p.x, dY = p.target.y - p.y;
                if (Math.sqrt(dX * dX + dY * dY) < 5) {
                    let d = p.damage; if (p.target.immuneToNormal && p.damageType !== 'siege') d = 0;
                    p.target.health -= d;
                    if (p.target.health <= 0) {
                        if (p.owner === 'player') { updatePlayerGold(p.target.reward); floatingTexts.push(new FloatingText(p.target.x, p.target.y, `+${p.target.reward}`, '#f1c40f')); }
                        else { updateGhostGold(p.target.reward); floatingTexts.push(new FloatingText(p.target.x, p.target.y, `+${p.target.reward}`, '#bdc3c7')); }
                    }
                    projectiles.splice(i, 1);
                }
            }
            const pL = monsters.filter(m => m.reachedEnd), gL = ghostMonsters.filter(m => m.reachedEnd);
            pL.forEach(m => updateGhostHealth(-m.damageToPlayer)); gL.forEach(m => updatePlayerHealth(-m.damageToPlayer));
            monsters = monsters.filter(m => m.health > 0 && !m.reachedEnd); ghostMonsters = ghostMonsters.filter(m => m.health > 0 && !m.reachedEnd);
    ...
    function handleMouseMove(e) {
        ...
        if (selectedAction && selectedAction.type === 'tower') {
            hoveredTower = null; if (col < 0 || row < 0) { selectedAction.ghostTower.visible = false; }
            else {
                selectedAction.ghostTower.visible = true; const p = camera.getTileCenter(col, row); selectedAction.ghostTower.x = p.x; selectedAction.ghostTower.y = p.y; selectedAction.ghostTower.col = col; selectedAction.ghostTower.row = row;
                const o = towerData[selectedAction.unit].levels[0], tw = camera.project(col, row).tileWidth; selectedAction.ghostTower.rangeInPixels = o.range * tw;
    ...
    async function saveGhost(act) {
        log("Tentando gravar ghost com " + act.length + " ações.");
        if (act.length === 0) return;
        act.sort((a, b) => (a.round !== b.round) ? a.round - b.round : a.timestamp - b.timestamp);
        try { 
            await db.collection("ghosts").add({ 
                actions: act, 
                timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                version: "2.0" 
            }); 
            log("GHOST GRAVADO NO FIREBASE COM SUCESSO!"); 
        } catch (e) { log("Erro Ghost: " + e); }
    }

    async function loadGhost() {
        try {
            const t = await db.collection("ghosts").orderBy("timestamp", "desc").limit(1).get();
            if (!t.empty) { ghost = t.docs[0].data(); ghostActions = ghost.actions || []; log("Ghost Firebase carregado."); }
            else { throw new Error("Vazio"); }
        } catch (e) {
            try { const res = await fetch(`ghost.json?v=${Date.now()}`); ghost = await res.json(); ghostActions = ghost.actions || []; log("Ghost Local carregado."); }
            catch (e2) { ghost = null; ghostActions = []; }
        }
        if (ghostActions.length > 0) ghostActions.sort((a, b) => (a.round !== b.round) ? a.round - b.round : a.timestamp - b.timestamp);
    }
