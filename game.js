...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function drawGameScene() {
        if (!camera) return; // Segurança: Não desenhar se a câmara não estiver pronta.
        drawGrid();
    ...
    class Monster{constructor(t,l,p){const c=monsterData[t].levels[l-1];this.type=t,this.level=l,this.path=p,this.pathIndex=0,this.health=c.health,this.maxHealth=c.health,this.speed=c.speed,this.reward=c.reward;const s=camera.getTileCenter(p[0].x,p[0].y);this.x=s.x,this.y=s.y,this.reachedEnd=!1}move(dT){if(this.reachedEnd||!dT)return;const tI=this.pathIndex,tP=this.path[tI],tC=camera.getTileCenter(tP.x,tP.y),dX=tC.x-this.x,dY=tC.y-this.y,dist=Math.sqrt(dX*dX+dY*dY),mD=this.speed*dT;if(dist<mD){this.pathIndex++;if(this.pathIndex>=this.path.length)this.reachedEnd=!0;else this.move(dT-dist/this.speed)}else{this.x+=dX/dist*mD,this.y+=dY/dist*mD}}draw(){ctx.fillStyle='red',ctx.beginPath(),ctx.arc(this.x,this.y,10,0,2*Math.PI),ctx.fill()}}
    ...
    function handleActionButtonClick(btn) {
        ...
        if (type === 'monster') { spawnPlayerMonster(unit); }
        else if (type === 'tower' && towerData && towerData[unit]) {
            if (selectedAction && selectedAction.button === btn) { btn.classList.remove("selected"); selectedAction = null; }
            else { if (selectedAction) { selectedAction.button.classList.remove("selected"); } btn.classList.add("selected"); selectedAction = { button: btn, type: type, unit: unit, ghostTower: { visible: false, x: 0, y: 0, col: -1, row: -1, range: 0, isValid: false } }; }
        }
        drawGameScene();
    }
    ...
    function startGameIfNeeded() {
        ...
        gameStarted = true;
        lastTime = null; // Inicia o contador de tempo AQUI
        if (ghost && ghostActions && ghostActions.length > 0) {
    ...
    function gameLoop(timestamp) {
        ...
        if (gameStarted) {
            ...
            if (ghost && nextGhostActionIndex !== -1) { while (nextGhostActionIndex < ghostActions.length && roundTime >= ghostActions[nextGhostActionIndex].timestamp) { const action = ghostActions[nextGhostActionIndex]; if (action.action === 'spawn') { spawnGhostMonster(action.type, action.level); } else if (action.action === 'build') { buildGhostTower(action.type, action.level, gridCols - 1 - action.col, gridRows - 1 - action.row); } nextGhostActionIndex++; } }
            ...
            for (let i = projectiles.length - 1; i >= 0; i--) { const p = projectiles[i]; p.move(dT); const dX = p.target.x - p.x, dY = p.target.y - p.y; if (Math.sqrt(dX*dX + dY*dY) < 5) { if(p.target) { p.target.health -= p.damage; if (p.target.health <= 0) { if (p.owner === 'player') updatePlayerGold(p.target.reward); else updateGhostGold(p.target.reward); } } projectiles.splice(i, 1); } else if (!p.target || p.target.health <= 0) { projectiles.splice(i, 1); } }
            ...
            if (ghost && nextGhostActionIndex >= ghostActions.length && monsters.length === 0 && ghostMonsters.length === 0) { endGame(true); }
        }
    ...
    async function main() { 
        ...
        camera.verticalScale = 0.95; // A nossa correção visual.
        await loadGameData();
        updateActionButtons(); 
        await resetGame();
        resize(); // Ajusta o canvas ao tamanho inicial.
        requestAnimationFrame(gameLoop); // Inicia o ciclo de renderização.
    }
