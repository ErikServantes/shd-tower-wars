...
document.addEventListener('DOMContentLoaded', () => {
    ...
    log('UI de Grelha Robusta Inicializada.');
    ...
    function resize(){const R=1/2,W=window.innerWidth/window.innerHeight;let w,h;if(W>R){h=window.innerHeight;w=h*R}else{w=window.innerWidth;h=w/R}canvas.width=w;canvas.height=h;drawGrid()}
    window.addEventListener('resize', resize);

    // --- Funções de Projeção e Desenho (Ajustadas para a UI) ---
    ...
    // --- Classes (Monster, Tower) - Omitidas por brevidade ---
    ...
    function showActionButtons(menuType) {
        if (menuType === 'towers') {
            towerActionButtons.forEach(btn => btn.classList.remove('hidden'));
            monsterActionButtons.forEach(btn => btn.classList.add('hidden'));
            towerMenuBtn.classList.add('active');
            monsterMenuBtn.classList.remove('active');
        } else { // monsters
            towerActionButtons.forEach(btn => btn.classList.add('hidden'));
            monsterActionButtons.forEach(btn => btn.classList.remove('hidden'));
            monsterMenuBtn.classList.add('active');
            towerMenuBtn.classList.remove('active');
        }
    ...
    function handleActionButtonClick(btn) {
        ...
        selectedAction = { type, unit };

        if (type === 'tower') log(`Modo construção: ${unit}.`);
        if (type === 'monster') {
            startGameIfNeeded();
            playerActions.push({ action: 'spawn', type: unit, timestamp: roundTime });
    ...
    function handleCanvasClick(e) {
        ...
        const rect=canvas.getBoundingClientRect(),cX=e.clientX-rect.left,cY=e.clientY-rect.top, {col,row}=screenToGrid(cX,cY);
        const TOWER_COST=100,canBuild=col>=0&&row>=15&&!path.some(p=>p.x===col&&p.y===row)&&!towers.some(t=>t.col===col&&t.row===row)&&gold>=TOWER_COST;
        if(canBuild){startGameIfNeeded();updateGold(-TOWER_COST);towers.push(new Tower(col,row));playerActions.push({action:'build',type:selectedAction.unit,col,row,timestamp:roundTime});log(`Torre ${selectedAction.unit} construída.`);}
        else{log("Construção inválida.");}
        allActionButtons.forEach(b => b.classList.remove('selected'));
        selectedAction = null;
    }

    // ============== CICLO, ESTADO E GHOSTS (Omitido) ==============
    ...
    function resetGame(){
        ...
        showActionButtons('towers'); // Abre os botões de torre por defeito
        resize();
    }
