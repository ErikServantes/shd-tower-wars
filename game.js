...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function handleKeyPress(e){"Escape"===e.key&&selectedAction&&(selectedAction.button.classList.remove("selected"),selectedAction=null,drawGameScene());const t=parseInt(e.key);if(isNaN(t)||t<1||t>7)return;const o=Object.keys(monsterData),n=o[t-1];n&&spawnPlayerMonster(n)}
    ...
    function handleCanvasClick(e){if(!selectedAction||"tower"!==selectedAction.type)return;const{col,row,isValid}=selectedAction.ghostTower;if(!isValid)return;const unitType=selectedAction.unit,level=1,config=towerData[unitType].levels[level-1];if(playerGold<config.cost)return;updatePlayerGold(-config.cost),towers.push(new Tower(unitType,level,col,row,'player')),playerActions.push({action:"build",type:unitType,level:level,col:col,row:row,timestamp:roundTime}),selectedAction.button.classList.remove("selected"),selectedAction=null;drawGameScene();}
    ...
    function startGameIfNeeded() {
        ...
        if (ghost && ghostActions && ghostActions.length > 0) {
            const firstSpawnAction = ghostActions.find(action => action.action === 'spawn');
            const firstSpawnTime = firstSpawnAction ? firstSpawnAction.timestamp : -1;

            if (firstSpawnTime !== -1) {
                ghostActions.forEach(action => {
                    if (action.action === 'build' && action.timestamp < firstSpawnTime) {
                ...
                ghostActions.forEach(action => {
                    action.timestamp = Math.max(0, action.timestamp - firstSpawnTime);
                });

                nextGhostActionIndex = ghostActions.findIndex(action => action.timestamp === 0);
            } else {
                nextGhostActionIndex = -1; 
            }
        }
    ...
    async function saveGhost(actions){if(actions.length>0){actions.sort((a,b)=>a.timestamp-b.timestamp);await db.collection("ghosts").add({actions:actions,timestamp:firebase.firestore.FieldValue.serverTimestamp()}).then(()=>log("Ghost salvo com sucesso.")).catch(e=>log("Erro ao salvar ghost: "+e))}else log("Nenhuma ação para salvar.")}
    async function loadGhost(){
        try {
            ...
            if (ghostActions) ghostActions.sort((a,b) => a.timestamp - b.timestamp);
            log("Ghost carregado do Firebase.");
        } catch(t) {
            log(`${t.message} A carregar ghost local...`);
            try {
                const res = await fetch("ghost.json");
                ...
                log(`Erro ao carregar ghost local: ${e.message}`),ghost=null,ghostActions=[]
            }
        }
    }
